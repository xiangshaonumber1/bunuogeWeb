<template>
  <div class="personalMessage">
    <Divider orientation="left" ><span class="p-message-title">我的消息</span></Divider>

    <!--主要内容-->
    <div ref="p-message-wrapper" class="p-message-wrapper">
      <Card class="p-message-card" v-for="message in personalMessageData" :key="message.messageID">
        <div>
...
<script>
    export default {
      ...
      methods:{
          ...
          if (this.loadMore){
            ...
            if (result.system_message_infos.length >0){
              ...
              for (let data of result.system_message_infos){
                this.personalMessageData.push(data)
              }
            }
            //并且只有当查询的总量 大于 当前已存在数据长度时，才允许加载下一页
            if (result.total > this.personalMessageData.length){
              this.page++;
              this.loadMore = true;
            }else {
              this.loadMore = false;
        ...
        wrapperListener(){
          let container = this.$refs['p-message-wrapper'];
          //div可视高度
